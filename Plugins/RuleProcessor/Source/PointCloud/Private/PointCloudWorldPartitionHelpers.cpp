...
bool CheckoutManagedActors(UWorld* World, const TArray<TSoftObjectPtr<AActor>>& ActorsToCheckout)
{
	...
	if (UWorldPartition* WorldPartition = World->GetWorldPartition())
	{
		...
		{
			if (const FWorldPartitionActorDesc* ActorDesc = WorldPartition->GetActorDesc(ManagedActor.ToSoftObjectPath()))
			{
...
bool DeleteManagedActors(UWorld* World, const TArray<TSoftObjectPtr<AActor>>& ActorsToDelete)
{
	...
	if (WorldPartition)
	{
		...
		{
			const FWorldPartitionActorDesc* ActorDesc = WorldPartition->GetActorDesc(ManagedActor.ToSoftObjectPath());
...
bool GetNewActorNameFromRecycledPackage(UWorld* World, TFunctionRef<TSoftObjectPtr<AActor>()> RecycleFn, FName& OutNewActorName)
{
	...
	if (WorldPartition)
	{
		...
		const FWorldPartitionActorDesc* ActorDesc = WorldPartition->GetActorDesc(ActorToReplace.ToSoftObjectPath());
...
bool MoveNewActorsToRecycledPackages(UWorld* World, TArray<TSoftObjectPtr<AActor>>& NewActors, TFunctionRef<TSoftObjectPtr<AActor>()> RecycleFn, FBox& OutActorBounds)
{
	...
	if (WorldPartition)
	{
		...
		{
			const FWorldPartitionActorDesc* ActorToRenameDesc = WorldPartition->GetActorDesc(ActorToRename.ToSoftObjectPath());

			TSoftObjectPtr<AActor> ActorToReplace = RecycleFn();
			const FWorldPartitionActorDesc* ActorToReplaceDesc = WorldPartition->GetActorDesc(ActorToReplace.ToSoftObjectPath());
...
bool MoveNewActorsToRecycledPackages(UWorld* World, TArray<TSoftObjectPtr<AActor>>& NewActors, TFunctionRef<TSoftObjectPtr<AActor>()> RecycleFn)
{
	...
	if (bMovedActors)
	{
		...
		{
			World->GetWorldPartition()->UnloadEditorCells(BoxToUnload, false);
		}
#endif
