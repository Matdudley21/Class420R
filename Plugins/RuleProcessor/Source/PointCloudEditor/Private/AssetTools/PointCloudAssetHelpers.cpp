...
TArray<FName> UPointCloudAssetsHelpers::GetSelectedRuleProcessorItemsFromContentBrowser()
{
	...
	{
		if(a.AssetClass.ToString() == PointCloudClass->GetName() || a.AssetClass.ToString() == PointCloudImplClass->GetName())
...
void UPointCloudAssetsHelpers::DeleteAllActorsOnDataLayer(UWorld* InWorld, const FString& InDataLayer)
{
	...
	if (InDataLayer.IsEmpty())
	{
		UE_LOG(PointCloudLog, Warning, TEXT("Invalid data layer name"));
		return;
	}

	FName DataLayerLabel = *InDataLayer;

	AWorldDataLayers* WorldDataLayers = World->GetWorldDataLayers();

	if (!WorldDataLayers)
	{
		UE_LOG(PointCloudLog, Warning, TEXT("World has no data layers"));
		return;
	}

	const UDataLayer* DataLayer = WorldDataLayers->GetDataLayerFromLabel(DataLayerLabel);
	...
	FWorldPartitionHelpers::ForEachActorDesc(WorldPartition, [&DataLayerName, &ActorsToDelete](const FWorldPartitionActorDesc* ActorDesc)
		{
			if (ActorDesc != nullptr && ActorDesc->GetDataLayers().Contains(DataLayerName))
			{
...
void UPointCloudAssetsHelpers::ParseModulesOnActor(AActor* InActor, const TArray<FString>& InDataLayerLabels, TArray<FPointCloudPoint>& OutPoints)
{
	if (!InActor)
	{
		return;
	...
	if (InDataLayerLabels.Num() > 0 && InActor->GetWorld() && InActor->GetWorld()->GetWorldDataLayers())
	{
		AWorldDataLayers* WorldDataLayers = InActor->GetWorld()->GetWorldDataLayers();
		for (const FString& DataLayerLabel : InDataLayerLabels)
		{
			DataLayers.Add(WorldDataLayers->GetDataLayerFromLabel(*DataLayerLabel));
		}
	}

	auto AddActorInfo = [InActor, &InDataLayerLabels, &DataLayers](TMap<FString, FString>& AttributesToEdit) {
		AttributesToEdit.Add(TEXT("ActorLabel"), InActor->GetActorLabel());
		AttributesToEdit.Add(TEXT("ActorName"), InActor->GetName());

		for (int DataLayerIndex = 0; DataLayerIndex < DataLayers.Num(); ++DataLayerIndex)
		{
			bool bActorInDataLayer = InActor->ContainsDataLayer(DataLayers[DataLayerIndex]);
			AttributesToEdit.Add(InDataLayerLabels[DataLayerIndex], bActorInDataLayer ? TEXT("1") : TEXT("0"));
		}
	};
	...
	else if (APackedLevelActor* PackedLevelActor = Cast<APackedLevelActor>(InActor))
	{
		...
		Point.Transform = InActor->GetTransform();
		AddActorInfo(Point.Attributes);
		Point.Attributes.Add(InstanceKey, FAssetData(PackedLevelActor->BlueprintAsset.Get()).GetExportTextName());
		Point.Attributes.Add(CustomDataKey, DefaultCustomDataValue);
	}
...
TArray<FPointCloudPoint> UPointCloudAssetsHelpers::GetModulesFromDataLayers(UWorld* InWorld, const TArray<FString>& InDataLayerNames)
{
	...
	for (const FString& DataLayerLabel : InDataLayerNames)
	{
		const UDataLayer* DataLayer = WorldDataLayers->GetDataLayerFromLabel(*DataLayerLabel);
		if (DataLayer)
		{
			DataLayerNames.Add(DataLayer->GetFName());
		}
		else
		{
			UE_LOG(PointCloudLog, Warning, TEXT("Data layer name does not match to any existing data layer"));
	...
	FWorldPartitionHelpers::ForEachActorDesc(WorldPartition, [&DataLayerNames, &ActorsToProcess](const FWorldPartitionActorDesc* ActorDesc)
		{
			if (ActorDesc)
			{
				bool bHasMatchingDataLayer = Algo::AnyOf(ActorDesc->GetDataLayers(), [&DataLayerNames](const FName& DLName) { return DataLayerNames.Contains(DLName); });
	...
	for (const FWorldPartitionActorDesc* ActorDesc : ActorsToProcess)
	{
		...
		ParseModulesOnActor(ActorDesc->GetActor(), InDataLayerNames, Points);
	}
...
TArray<FPointCloudPoint> UPointCloudAssetsHelpers::GetModulesFromMapping(USliceAndDiceMapping* InMapping)
{
	...
	const TArray<FString> DummyDataLayerLabels;
	...
	for (const TSoftObjectPtr<AActor>& Actor : ActorsToProcess)
	{
		...
		if (WorldPartition)
		{
			...
			const FWorldPartitionActorDesc* ActorDesc = WorldPartition->GetActorDesc(Actor.ToSoftObjectPath());
			FWorldPartitionReference ActorRef(WorldPartition, ActorDesc->GetGuid());
			ParseModulesOnActor(ActorDesc->GetActor(), DummyDataLayerLabels, Points);
		}
		else
		{
			ParseModulesOnActor(Actor.Get(), DummyDataLayerLabels, Points);
		}
	}
...
void UPointCloudAssetsHelpers::ExportToAlembic(const FString& InFilename, const TArray<FPointCloudPoint>& InPoints)
{
	...
	for (int32 i = 0; i != NumPoints; ++i)
	{
		...
		Rotations[i] = FQuat(Transform.GetRotation().X, Transform.GetRotation().Z, Transform.GetRotation().Y, -Transform.GetRotation().Z);
		Translations[i] = FVector(Transform.GetTranslation().X, Transform.GetTranslation().Z, Transform.GetTranslation().Y);
		Scales[i] = FVector(Transform.GetScale3D().X, Transform.GetScale3D().Z, Transform.GetScale3D().Y);
