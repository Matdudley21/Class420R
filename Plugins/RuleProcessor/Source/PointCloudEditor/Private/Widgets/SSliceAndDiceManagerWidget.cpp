...
TSharedPtr<SWidget> SSliceAndDiceManagerWidget::OnOpenContextMenu()
{
	...
	if (bHasSelection)
	{
		...
		Section.AddMenuEntry("MoveMappingToDifferentManager",
			...
			FSlateIcon(FEditorStyle::GetStyleSetName(), "Icons.CircleArrowRight"),
			FUIAction(
		...
		Section.AddMenuEntry("RemoveSelectedMappings",
			...
			FSlateIcon(FEditorStyle::GetStyleSetName(), "Icons.X"),
			FUIAction(
...
void SSliceAndDiceManagerWidget::ShowManagedActorsList(const TArray<USliceAndDiceMapping*>& InMappings)
{
	...
	auto GetActorLabel = [WorldPartition](const TSoftObjectPtr<AActor>& InActor) -> FString {
		if (WorldPartition)
		{
			if (const FWorldPartitionActorDesc* ActorDesc = WorldPartition->GetActorDesc(InActor.ToSoftObjectPath()))
			{
...
void SSliceAndDiceManagerWidget::DeleteManagedActors(bool bCleanDisabled)
{
	...
	if (bCleanDisabled)
	{
		const FText Title = FText::FromString(TEXT("Delete Managed Actors?"));
...
void SSliceAndDiceManagerWidget::ClearDataLayer()
{
	...
	FName SelectedDataLayerLabel;

	if (GetDataLayerName(nullptr, World, SelectedDataLayerLabel))
	{
		UPointCloudAssetsHelpers::DeleteAllActorsOnDataLayer(World, SelectedDataLayerLabel.ToString());
	}
}
