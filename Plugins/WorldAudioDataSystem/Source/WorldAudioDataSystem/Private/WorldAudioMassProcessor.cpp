...
void UWorldAudioMassProcessor::Initialize(UObject& Owner)
{
...
void UWorldAudioMassProcessor::Execute(UMassEntitySubsystem& EntitySubsystem, FMassExecutionContext& Context)
{
	...
	// Find traffic vehicle agents
	{
		...
		NearbyTrafficVehicleEntityQuery.ForEachEntityChunk(EntitySubsystem, Context, [this](FMassExecutionContext& QueryContext)
		{
	...
	{
		{
			...
			// Find crowd agents
			CrowdAgentEntityQuery.ForEachEntityChunk(EntitySubsystem, Context, [this](FMassExecutionContext& QueryContext)
			{
				const TConstArrayView<FTransformFragment> TransformFragments = QueryContext.GetFragmentView<FTransformFragment>();
