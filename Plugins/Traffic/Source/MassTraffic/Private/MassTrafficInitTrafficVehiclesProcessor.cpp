...
void UMassTrafficInitTrafficVehiclesProcessor::Initialize(UObject& InOwner)
{
	...
	ReplicationSubsystem = UWorld::GetSubsystem<UMassReplicationSubsystem>(InOwner.GetWorld());
}

void UMassTrafficInitTrafficVehiclesProcessor::InitNetIds(UMassEntitySubsystem& EntitySubsystem, FMassExecutionContext& Context)
{
	QUICK_SCOPE_CYCLE_COUNTER(MassProcessor_InitNetworkID_Run);

	check(EntitySubsystem.GetWorld() && EntitySubsystem.GetWorld()->GetNetMode() != NM_Client);
...
void UMassTrafficInitTrafficVehiclesProcessor::Execute(UMassEntitySubsystem& EntitySubsystem, FMassExecutionContext& Context)
{
	InitNetIds(EntitySubsystem, Context);
