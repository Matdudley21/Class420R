...
void UMassTrafficTrailerUpdateCustomVisualizationProcessor::Initialize(UObject& Owner)
{
	Super::Initialize(Owner);

	World = Owner.GetWorld();
	check(World);
	MassTrafficSubsystem = UWorld::GetSubsystem<UMassTrafficSubsystem>(World);
	check(MassTrafficSubsystem);
}

void UMassTrafficTrailerUpdateCustomVisualizationProcessor::Execute(UMassEntitySubsystem& EntitySubsystem, FMassExecutionContext& Context)
{
	...
	EntityQuery.ForEachEntityChunk(EntitySubsystem, Context, [this, &EntitySubsystem](FMassExecutionContext& QueryContext)
	{
		...
		for (int32 EntityIndex = 0; EntityIndex < NumEntities; EntityIndex++)
		{
			...
			FMassEntityView VehicleMassEntityView(EntitySubsystem, ConstrainedVehicleFragment.Vehicle);
			if (!ensure(EntitySubsystem.IsEntityValid(ConstrainedVehicleFragment.Vehicle)))
			{
				continue;
			}
			...
			switch (RepresentationFragment.CurrentRepresentation)
			{
				case EMassRepresentationType::LowResSpawnedActor:
				{
					if (AActor* Actor = ActorFragment.GetMutable())
					{
						...
						QueryContext.Defer().EmplaceCommand<FDeferredCommand>([Actor, NewActorTransform = TransformFragment.GetTransform()](UMassEntitySubsystem& System)
						{
						...
						if (!SimpleVehiclePhysicsFragments.IsEmpty())
						{
							...
							if (MassTrafficVehicleComponent)
							{
								...
								QueryContext.Defer().EmplaceCommand<FDeferredCommand>([MassTrafficVehicleComponent, Entity = QueryContext.GetEntity(EntityIndex)](UMassEntitySubsystem& CallbackEntitySubsystem)
								{
