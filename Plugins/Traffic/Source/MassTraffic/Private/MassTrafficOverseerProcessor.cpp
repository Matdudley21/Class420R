...
void UMassTrafficOverseerProcessor::Execute(UMassEntitySubsystem& EntitySubsystem, FMassExecutionContext& Context)
{
...

	{
		...
		for (FMassTrafficZoneGraphData* TrafficZoneGraphData : MassTrafficSubsystem->GetMutableTrafficZoneGraphData())
		{
...

	{
		...
		RecyclableTrafficVehicleEntityQuery.ForEachEntityChunk(EntitySubsystem, Context, [&EntitySubsystem, World, this](FMassExecutionContext& QueryContext)
		{
			...
			for (int32 EntityIndex = 0; EntityIndex < NumEntities; ++EntityIndex)
			{
				...
				{
					LaneChangeFragment.EndLaneChangeProgression(VehicleLightsFragment, NextVehicleFragment, EntitySubsystem);
				}
				...
				if (!UE::MassTraffic::FindNearbyVehiclesOnLane_RelativeToVehicleEntity(&Vehicle_CurrentLane, RecyclableTrafficVehicle, NextVehicleFragment, PreviousVehicleOnLane, NextVehicleOnLane, EntitySubsystem, /*VisLogOwner*/MassTrafficSubsystem.Get()))
				{
					// Error condition. Try again next time.
					continue;
				}
				FMassTrafficNextVehicleFragment* PreviousVehicleOnLane_NextVehicleFragment = PreviousVehicleOnLane.IsSet() ? &EntitySubsystem.GetFragmentDataChecked<FMassTrafficNextVehicleFragment>(PreviousVehicleOnLane) : nullptr;

				// Try and move the vehicle to one of the least busiest lanes off screen
				const bool bTransferred = MoveVehicleToFreeSpaceOnRandomLane(EntitySubsystem, *ZoneGraphStorage,
				...
				// Debug
				UE::MassTraffic::DrawDebugDensityManagementRecyclableVehicle(World, TransformFragment.GetTransform().GetLocation(), bTransferred, /*bVisLog*/false, MassTrafficSubsystem.Get());
				...
				if (bTransferred)
				{
					...
					QueryContext.Defer().PushCommand(FCommandSwapTags(RecyclableTrafficVehicle, FMassTrafficRecyclableVehicleTag::StaticStruct(), FMassTrafficVehicleTag::StaticStruct()));
				}
			}
		});
	}

	{
		...
		for (FZoneGraphTrafficLaneData* BusiestLane : BusiestLanes)
		{
			...
			BusiestLane->ForEachVehicleOnLane(EntitySubsystem, [&](const FMassEntityView& BusiestLaneVehicle_EntityView, struct FMassTrafficNextVehicleFragment& BusiestLaneVehicle_NextVehicleFragment, struct FMassZoneGraphLaneLocationFragment& BusiestLaneVehicle_LaneLocationFragment)
			{
			...
			for (int32 BusiestLaneVehicleIndex = 0; BusiestLaneVehicleIndex < BusiestLaneVehiclesToTransfer.Num(); ++BusiestLaneVehicleIndex)
			{
				...
				const FZoneGraphStorage* ZoneGraphStorage = ZoneGraphSubsystem->GetZoneGraphStorage(BusiestLaneVehicle_LaneLocationFragment.LaneHandle.DataHandle);
				check(ZoneGraphStorage);
				...
				if (NextVehicleOnBusiestLane.IsSet() && EntitySubsystem.GetFragmentDataChecked<FMassZoneGraphLaneLocationFragment>(NextVehicleOnBusiestLane).LaneHandle != BusiestLane->LaneHandle)
				{
...
) const
{
	...
	for (int32 i = 0; i < CandidateLanes.Num() && !bTransferred; ++i)
	{
		...
		else
		{
			...
			CandidateLane.ForEachVehicleOnLane(EntitySubsystem, [&](const FMassEntityView& CandidateLaneVehicle_EntityView, const FMassTrafficNextVehicleFragment& CandidateLaneVehicle_NextVehicleFragment, const FMassZoneGraphLaneLocationFragment& CandidateLaneVehicle_LaneLocationFragment)
			{
			...
			if (CandidateLaneVehicleBehind_EntityView.IsSet())
			{
				...
				{
					FMassEntityView NextMassEntityView(EntitySubsystem, CandidateLaneVehicle_NextVehicle);
