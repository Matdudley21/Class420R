...
)
{
	...
	if (NextTrafficLaneData->ConstData.bIsIntersectionLane)
	{
		...
		const float PostIntersectionSpaceAvailable = PostIntersectionTrafficLaneData->SpaceAvailableFromStartOfLaneForVehicle(EntitySubsystem, true, false); // (See all INTERSTRAND1.)
	...
	FMassTrafficVehicleLaneChangeFragment* LaneChangeFragment, bool& bIsVehicleStuck)
{
	...
	if (NewCurrentLane.TailVehicle.IsSet())
	{
		...
		const FMassEntityView NextVehicleView(EntitySubsystem, NextVehicleFragment.GetNextVehicle());
	...
	{
		const FTransformFragment& TransformFragment = EntitySubsystem.GetFragmentDataChecked<FTransformFragment>(VehicleEntity);
		const FMassEntityHandle NearestNextVehicle = FindNearestTailVehicleOnNextLanes(NewCurrentLane, TransformFragment.GetTransform().GetLocation(), EntitySubsystem, EMassTrafficFindNextLaneVehicleType::Tail);
	...
	// 'Lane changing' next vehicles..
	{
		...
		if (NewCurrentLane.GhostTailVehicle_FromLaneChangingVehicle.IsSet())
		{
			FMassTrafficVehicleLaneChangeFragment* LaneChangeFragment_GhostTailEntity =
				EntitySubsystem.GetFragmentDataPtr<FMassTrafficVehicleLaneChangeFragment>(NewCurrentLane.GhostTailVehicle_FromLaneChangingVehicle);
			
			if (LaneChangeFragment_GhostTailEntity && LaneChangeFragment_GhostTailEntity->IsLaneChangeInProgress())
			{
				LaneChangeFragment_GhostTailEntity->AddOtherLaneChangeNextVehicle_ForVehicleBehind(VehicleEntity, EntitySubsystem);
			}
	...
	{
		NewCurrentLane.ForEachVehicleOnLane(EntitySubsystem, [VehicleEntity](const FMassEntityView& VehicleMassEntityView, struct FMassTrafficNextVehicleFragment& NextVehicleFragment, struct FMassZoneGraphLaneLocationFragment& LaneLocationFragment)
		{
...
)
{
	...
	// of it's lane. It will get a new next vehicle once it begins to go through the intersection.
	{
		...
		Lane_Chosen.ForEachVehicleOnLane(EntitySubsystem, [Entity_Current](const FMassEntityView& VehicleMassEntityView, struct FMassTrafficNextVehicleFragment& NextVehicleFragment, struct FMassZoneGraphLaneLocationFragment& LaneLocationFragment)
		{
