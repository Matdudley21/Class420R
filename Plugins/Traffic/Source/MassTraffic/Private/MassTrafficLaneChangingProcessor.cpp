...
static void TryStartingNewLaneChange(
	...
	UMassEntitySubsystem& EntitySubsystem, const FZoneGraphStorage& ZoneGraphStorage)
{
	...
	if (!FindNearbyVehiclesOnLane_RelativeToDistanceAlongLane(Lane_Chosen, DistanceAlongLane_Chosen, /*out*/Entity_Chosen_Behind, /*out*/Entity_Chosen_Ahead, /*Coordinator,*/ EntitySubsystem))
	{
	...
	{
		FMassEntityView EntityView(EntitySubsystem, Entity_Chosen_Behind);
	...
	{
		FMassEntityView EntityView(EntitySubsystem, Entity_Chosen_Ahead);
	...
	if (!FindNearbyVehiclesOnLane_RelativeToVehicleEntity(Lane_Current, Entity_Current, NextVehicleFragment_Current, /*out*/Entity_Current_Behind, /*out*/Entity_Current_Ahead, EntitySubsystem, /*VisLogOwner*/&MassTrafficSubsystem))
	{
		// Error condition. Try again next time.
	...
	{
		FMassEntityView VehicleEntityView(EntitySubsystem, Entity_Current_Behind);
	...
	{
		FMassEntityView VehicleEntityView(EntitySubsystem, Entity_Current_Ahead);
	...
	{
		FMassEntityHandle Entity_NewNextVehicle = FindNearestTailVehicleOnNextLanes(*Lane_Chosen, Position_Chosen, EntitySubsystem, EMassTrafficFindNextLaneVehicleType::Tail);
	...
	const FRandomStream& RandomStream)
{
	...
	LaneChangeFragment_Current.UpdateLaneChange(
		...
		EntitySubsystem, MassTrafficSettings, RandomStream);
}
...
/*virtual*/ void UMassTrafficLaneChangingProcessor::ConfigureQueries() 
{
...
/*virtual*/ void UMassTrafficLaneChangingProcessor::Execute(UMassEntitySubsystem& EntitySubsystem, FMassExecutionContext& Context)
{
...

	{
		...
		StartNewLaneChangesEntityQuery_Conditional.ForEachEntityChunk(EntitySubsystem, Context, [&](FMassExecutionContext& ComponentSystemExecutionContext)
			{
				...
				for (int32 EntityIndex = 0; EntityIndex < Context.GetNumEntities(); EntityIndex++)
				{
					...
					TryStartingNewLaneChange(Entity, Context,
						...
						LaneChangeFragment,
						AvoidanceFragment,
						bVisLog, *MassTrafficSubsystem, *MassTrafficSettings, RandomStream, EntitySubsystem, *ZoneGraphStorage);
				}
			});
	}
...

	{
		...
		UpdateLaneChangesEntityQuery_Conditional.ForEachEntityChunk(
				EntitySubsystem, Context, [&](FMassExecutionContext& ComponentSystemExecutionContext)
			{
				...
				for (int32 EntityIndex = 0; EntityIndex < Context.GetNumEntities(); EntityIndex++)
				{
					...
					UpdateLaneChange(
						...
						LaneChangeFragment,
						NextVehicleFragment,
						SimulationVariableTickFragment.DeltaTime, EntitySubsystem, *MassTrafficSettings, RandomStream);
				}
			});
	}
}
