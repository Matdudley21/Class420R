...
namespace
{
		...
		const UMassEntitySubsystem& EntitySubsystem)
	{
		...
		{
			const FMassEntityView NextVehicleEntityView(EntitySubsystem, NextVehicleFragment.GetNextVehicle());
...
void UMassTrafficVehicleControlProcessor::Execute(UMassEntitySubsystem& EntitySubsystem, FMassExecutionContext& Context)
{
	// Advance simple agents
	SimpleVehicleControlEntityQuery_Conditional.ForEachEntityChunk(EntitySubsystem, Context, [&](FMassExecutionContext& ComponentSystemExecutionContext)
		{
	...
	PIDVehicleControlEntityQuery_Conditional.ForEachEntityChunk(EntitySubsystem, Context, [&](FMassExecutionContext& ComponentSystemExecutionContext)
		{
			const TConstArrayView<FMassSimulationVariableTickFragment> VariableTickFragments = Context.GetFragmentView<FMassSimulationVariableTickFragment>();
...
) const
{
	...
	if (!bIsOffLOD /*EDGE CASE-ish - don't do this in off-LOD*/ && bVehicleCantStopAtLaneExit) 
	{
		...
		SetVehicleCantStopAtLaneExit(VehicleControlFragment, LaneLocationFragment, NextVehicleFragment, EntitySubsystem);
	}
	...
	UE::MassTraffic::DrawDebugSpeed(
		...
		bVisLog, MassTrafficSubsystem.Get());
}
