...
void UMassTrafficPostPhysicsUpdateTrafficVehiclesProcessor::Execute(UMassEntitySubsystem& EntitySubsystem, FMassExecutionContext& Context)
{
	// Advance agents
	PIDControlTrafficVehicleQuery.ForEachEntityChunk(EntitySubsystem, Context, [&](FMassExecutionContext& ComponentSystemExecutionContext)
	{
		...
		for (int32 Index = 0; Index < NumEntities; ++Index)
		{
			...
			else
			{
					...
					UE_VLOG_LOCATION(MassTrafficSubsystem.Get(), LogMassTraffic, Error, TransformFragment.GetTransform().GetLocation(), 50.0f, FColor::Red, TEXT("PostPhysicsUpdateTrafficVehicles FindNearestLocationOnLane failed"));
				#endif
			}
