...
void UMassTrafficVehicleVisualizationLODProcessor::Execute(UMassEntitySubsystem& EntitySubsystem, FMassExecutionContext& Context)
{
		...
		if (GMassTrafficDebugViewerLOD)
		{
			...
			DebugEntityQuery.ForEachEntityChunk(EntitySubsystem, Context, [this](FMassExecutionContext& Context)
			{
				...
				for (int EntityIdx = 0; EntityIdx < NumEntities; EntityIdx++)
				{
					...
					{
						UE_VLOG_LOCATION(MassTrafficSubsystem.Get(), TEXT("MassTraffic Viewer LOD"), Log, EntityLocation.GetTransform().GetLocation() + FVector(0.0f, 0.0f, 200.0f), /*Radius*/ 10.0f, UE::MassLOD::LODColors[ViewerLODIdx], TEXT("%d %d"), ViewerLODIdx, Context.GetEntity(EntityIdx).Index);
					}
				}
			});
		}
