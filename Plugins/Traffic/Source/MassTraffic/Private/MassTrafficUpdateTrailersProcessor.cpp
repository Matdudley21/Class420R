...
void UMassTrafficUpdateTrailersProcessor::Execute(UMassEntitySubsystem& EntitySubsystem, FMassExecutionContext& Context)
{
	// Advance agents
		...
		for (int32 EntityIndex = 0; EntityIndex < NumEntities; ++EntityIndex)
		{
			...
			if (SimulationLODFragment.LOD <= EMassLOD::Medium)
			{
				...
				if (SimulationLODFragment.PrevLOD >= EMassLOD::Low)
				{
					...
					{
						if (PhysicsParams.Template && PhysicsParams.Template->SimpleVehiclePhysicsFragmentTemplate.IsValid())
						{
							Context.Defer().PushCommand(FCommandAddFragmentInstance(Context.GetEntity(EntityIndex), PhysicsParams.Template->SimpleVehiclePhysicsFragmentTemplate));
						}
					}
				}
			}
