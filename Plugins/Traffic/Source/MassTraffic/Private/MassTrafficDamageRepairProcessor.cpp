...
void UMassTrafficDamageRepairProcessor::Execute(UMassEntitySubsystem& EntitySubsystem, FMassExecutionContext& Context)
{
	// Skip damage repair?
	...
	DamagedVehicleEntityQuery.ForEachEntityChunk(EntitySubsystem, Context, [](FMassExecutionContext& Context)
	{
				...
				if (AActor* Actor = ActorFragment.GetOwnedByMassMutable())
				{
					...
					if (RepresentationLODFragment.LOD != EMassLOD::High)
					{
						...
						if (VehicleDamageFragment.VehicleDamageState < EMassTrafficVehicleDamageState::Repairing)
						{
							...
							{
								Context.Defer().EmplaceCommand<FDeferredCommand>([Actor](UMassEntitySubsystem&)
								{
					...
					if (VehicleDamageFragment.VehicleDamageState == EMassTrafficVehicleDamageState::Irreparable)
					{
						...
						else
						{
							...
							Context.Defer().PushCommand(FCommandSwapTags(Context.GetEntity(EntityIndex), FMassTrafficVehicleTag::StaticStruct(), FMassTrafficRecyclableVehicleTag::StaticStruct()));
						}
					}
				}
				// No actor
