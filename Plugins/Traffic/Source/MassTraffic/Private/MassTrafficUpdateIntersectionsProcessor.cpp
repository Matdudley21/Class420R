...
void UMassTrafficUpdateIntersectionsProcessor::Execute(UMassEntitySubsystem& EntitySubsystem, FMassExecutionContext& Context)
{
	...
	// Get Mass ZoneGraph & Crowd subsystems
	...
	EntityQuery.ForEachEntityChunk(EntitySubsystem, Context, [&](FMassExecutionContext& QueryContext)
	{
		...
		for (int32 Index = 0; Index < NumEntities; ++Index)
		{
			...
			if (IntersectionFragment.Periods.IsEmpty())
			{
				...
				if (bDoDrawDebug)
				{
					DrawDebugNumberOfPedestrians(GetWorld(), IntersectionFragment, *ZoneGraphStorage, MassCrowdSubsystem, TransformFragment.GetTransform().GetLocation());
				}
			#endif
