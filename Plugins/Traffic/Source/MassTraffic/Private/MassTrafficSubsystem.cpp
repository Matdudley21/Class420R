...
const FZoneGraphTrafficLaneData* UMassTrafficSubsystem::GetTrafficLaneData(const FZoneGraphLaneHandle LaneHandle) const
{
	...
	return TrafficZoneGraphData->GetTrafficLaneData(LaneHandle);
}
...
FZoneGraphTrafficLaneData* UMassTrafficSubsystem::GetMutableTrafficLaneData(const FZoneGraphLaneHandle LaneHandle)
{
	...
	return TrafficZoneGraphData->GetMutableTrafficLaneData(LaneHandle);
}

int32 UMassTrafficSubsystem::GetNumTrafficVehicleAgents()
{
	return TrafficVehicleEntityQuery.GetNumMatchingEntities(*MassEntitySubsystem);
}

bool UMassTrafficSubsystem::HasTrafficVehicleAgents()
{
	return TrafficVehicleEntityQuery.HasMatchingEntities(*MassEntitySubsystem);
}

int32 UMassTrafficSubsystem::GetNumParkedVehicleAgents()
{
	return ParkedVehicleEntityQuery.GetNumMatchingEntities(*MassEntitySubsystem);
}

bool UMassTrafficSubsystem::HasParkedVehicleAgents()
{
	return ParkedVehicleEntityQuery.HasMatchingEntities(*MassEntitySubsystem);
}
...
void UMassTrafficSubsystem::GetAllObstacleLocations(TArray<FVector> & ObstacleLocations)
{
	...
	FMassExecutionContext ExecutionContext(0.0f);
	ObstacleEntityQuery.ForEachEntityChunk(*MassEntitySubsystem, ExecutionContext, [&ObstacleLocations](FMassExecutionContext& QueryContext)
	{
...
{
	FMassExecutionContext ExecutionContext(0.0f);
	PlayerVehicleEntityQuery.ForEachEntityChunk(*MassEntitySubsystem, ExecutionContext, [&OutPlayerVehicleAgents](FMassExecutionContext& QueryContext)
	{
...
void UMassTrafficSubsystem::RemoveVehiclesOverlappingPlayers()
{
	...
	FMassProcessingContext ProcessingContext(*MassEntitySubsystem, 0.0f);
...
const FMassTrafficSimpleVehiclePhysicsTemplate* UMassTrafficSubsystem::GetOrExtractVehiclePhysicsTemplate(TSubclassOf<AWheeledVehiclePawn> PhysicsVehicleTemplateActor)
{
	...
	// Create a template physics sim fragment using the vehicle type's physics configs
	NewVehiclePhysicsTemplate->SimpleVehiclePhysicsFragmentTemplate.InitializeAs<FMassTrafficVehiclePhysicsFragment>();
	FMassTrafficVehiclePhysicsFragment& SimplePhysicsVehicleFragment = NewVehiclePhysicsTemplate->SimpleVehiclePhysicsFragmentTemplate.GetMutable<FMassTrafficVehiclePhysicsFragment>();
	...
	if (TempPhysicsActor)
	{
		...
		UE::MassTraffic::ExtractPhysicsVehicleConfig(
			...
			SimplePhysicsVehicleFragment.VehicleSim
		);
