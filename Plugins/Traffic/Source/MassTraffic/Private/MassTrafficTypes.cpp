...
void FZoneGraphTrafficLaneData::ForEachVehicleOnLane(const UMassEntitySubsystem& EntitySubsystem, FTrafficVehicleExecuteFunction Function) const
{
	...
	while (VehicleEntity.IsSet())
	{
		...
		{
			const UMassTrafficSubsystem* MassTrafficSubsystem = UWorld::GetSubsystem<UMassTrafficSubsystem>(EntitySubsystem.GetWorld());
			UE::MassTraffic::VisLogMalformedNextLaneLinks(EntitySubsystem, LaneHandle.Index, TailVehicle, FMassEntityHandle(), /*MarchEjectAt*/10000, MassTrafficSubsystem);
...
float FZoneGraphTrafficLaneData::SpaceAvailableFromStartOfLaneForVehicle(const UMassEntitySubsystem& EntitySubsystem, const bool bCheckLaneChangeGhostVehicles, const bool bCheckSplittingAndMergingGhostTailVehicles) const 
{
