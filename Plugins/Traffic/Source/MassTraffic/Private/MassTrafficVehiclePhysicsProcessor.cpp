...
void UMassTrafficVehiclePhysicsProcessor::Execute(UMassEntitySubsystem& EntitySubsystem, FMassExecutionContext& Context)
{
	TRACE_CPUPROFILER_EVENT_SCOPE(TEXT("SimplePhysicsVehicle"))

	// Get Chaos solver settings
	const int32 NumChaosConstraintSolverIterations = UPhysicsSettingsCore::Get()->SolverOptions.Iterations;
	...
	// Advance agents
	{
		...
		SimplePhysicsVehiclesQuery.ForEachEntityChunk(EntitySubsystem, Context, [&](FMassExecutionContext& QueryContext)
		{
			...
			for (int32 Index = 0; Index < NumEntities; ++Index)
			{
				...
				if (!TrailerConstraintFragments.IsEmpty())
				{
					...
					{
						FMassEntityView TrailerMassEntityView(EntitySubsystem, TrailerConstraintFragment.Trailer);
						...
						if (TrailerSimplePhysicsVehicleFragmentPtr)
						{
							...
							for (int Iteration = 0; Iteration < NumChaosConstraintSolverIterations; ++Iteration)
							{
								...
								{
									for (int32 PairIteration = 0; PairIteration < ChaosConstraintSolverSettings.ApplyPairIterations; ++PairIteration)
									{
...
)
{
	...
	UE::MassTraffic::DrawDebugSleepState(GetWorld(), VehicleWorldTransform.GetLocation(), bIsSleeping, bVisLog, MassTrafficSubsystem.Get());
#endif
	...
	FColor Color)
{
	...
	for (int WheelIndex = 0; WheelIndex < SimplePhysicsVehicleFragment.VehicleSim.SuspensionSims.Num(); WheelIndex++)
	{
		...
		if (bVisLog)
		{
			UE_VLOG_SEGMENT_THICK(MassTrafficSubsystem.Get(), TEXT("MassTraffic Suspension"), Verbose, OutHitResult.TraceStart, OutHitResult.TraceEnd, Color, 4.0f, TEXT("%d trace"), WheelIndex);
		}
		...
		if (OutHitResult.bBlockingHit)
		{
			if (bVisLog)
			{
				UE_VLOG_LOCATION(MassTrafficSubsystem.Get(), TEXT("MassTraffic Suspension"), Verbose, OutHitResult.ImpactPoint, 5.0f, Color, TEXT("%d hit"), WheelIndex);
			}
...
)
{
	...
	// @see UChaosVehicleSimulation::ApplyAerodynamics
	{
		...
		AddForce(WorldLiftDragForce, TotalForce, bVisLog, MassTrafficSubsystem.Get(), VehicleWorldTransform.GetLocation(), TEXT("Ae"));
	}
	...
	// @see UChaosWheeledVehicleSimulation::ApplySuspensionForces
	{
		...
		for (int WheelIndex = 0; WheelIndex < SimplePhysicsVehicleFragment.VehicleSim.WheelSims.Num(); WheelIndex++)
		{
			...
			if (PWheel.InContact())
			{
				...
				if (bVisLog)
				{
					UE_VLOG_SEGMENT(MassTrafficSubsystem.Get(), TEXT("MassTraffic Physics"), VeryVerbose, WheelWorldLocations[WheelIndex], WheelWorldLocations[WheelIndex] + ForceMagnitude * GMassTrafficDebugForceScaling * -VehicleWorldUpAxis, FColor::Purple, TEXT("FM: %f"), ForceMagnitude);
				}
			}
			else
			{
...

		{
			for (auto& Axle : SimplePhysicsVehicleFragment.VehicleSim.AxleSims)
			{
				...
				if (Axle.Setup.WheelIndex.Num() == 2)
				{
					...
					AddForceAtPosition(VehicleWorldCenterOfMass, ForceVector0, SusApplicationPoint0, TotalForce, TotalTorque, bVisLog, MassTrafficSubsystem.Get(), TEXT("Ax1"));
					...
					AddForceAtPosition(VehicleWorldCenterOfMass, ForceVector1, SusApplicationPoint1, TotalForce, TotalTorque, bVisLog, MassTrafficSubsystem.Get(), TEXT("Ax2"));
				}
			}
		}
	}
	...
	// @see UChaosWheeledVehicleSimulation::ApplyWheelFrictionForces
	{
		...
		for (int WheelIndex = 0; WheelIndex < SimplePhysicsVehicleFragment.VehicleSim.WheelSims.Num(); WheelIndex++)
		{
			...
			if (PWheel.InContact())
			{
				...
				AddForceAtPosition(VehicleWorldCenterOfMass, FrictionForceVector, WheelWorldLocation, TotalForce, TotalTorque, bVisLog, MassTrafficSubsystem.Get(), TEXT("F"));
			}
			else
			{
	...
	if (bVisLog)
	{
		...
		UE_VLOG_LOCATION(MassTrafficSubsystem.Get(), TEXT("MassTraffic Physics"), Log, VehicleWorldCenterOfMass + Offset, /*Radius*/5.0f, FColor::Red,
			TEXT("Velocity: %s\n")
			...
			*AngularVelocityFragment.AngularVelocity.ToString(),
			...
			TotalForce.Size(), 
			TotalTorque.Size());
		UE_VLOG_ARROW(MassTrafficSubsystem.Get(), TEXT("MassTraffic Physics"), Log, VehicleWorldCenterOfMass, VehicleWorldCenterOfMass + TotalForce * GMassTrafficDebugForceScaling, FColor::Red, TEXT("TF"));
		UE_VLOG_ARROW(MassTrafficSubsystem.Get(), TEXT("MassTraffic Physics"), Log, VehicleWorldCenterOfMass, VehicleWorldCenterOfMass + TotalTorque * GMassTrafficDebugForceScaling, FColor::Green, TEXT("TT"));
		for (uint16 WheelIndex = 0; WheelIndex < SimplePhysicsVehicleFragment.VehicleSim.WheelSims.Num(); ++WheelIndex)
		{
			Chaos::FSimpleWheelSim& Wheel = SimplePhysicsVehicleFragment.VehicleSim.WheelSims[WheelIndex];
			const FVector& WheelWorldLocation = WheelWorldLocations[WheelIndex];
			// UE_VLOG_LOCATION(MassTrafficSubsystem.Get(), TEXT("MassTraffic Physics"), Log, WheelWorldLocation, Wheel.GetEffectiveRadius(), Wheel.InContact() ? FColor::Cyan : FColor::Orange, TEXT("%d %s"), WheelIndex, *WheelLocalVelocities[WheelIndex].ToString());
			...
			UE_VLOG_ARROW(MassTrafficSubsystem.Get(), TEXT("MassTraffic Physics"), Log, WheelWorldLocation, WheelWorldLocation + WheelWorldForward * Wheel.GetEffectiveRadius(), FColor::Black, TEXT(""));
			// UE_VLOG_ARROW(MassTrafficSubsystem.Get(), TEXT("MassTraffic Physics"), Log, WheelWorldLocation, WheelWorldLocation + WheelWorldVelocities[WheelIndex] * GMassTrafficDebugForceScaling, FColor::Red, TEXT(""));
		}
	}
	...
	
	{
		...
		AddForceAtPosition(VehicleWorldCenterOfMass, GravityForceForPeripheralMass, VehicleWorldPeripheralCenterOfMass, TotalForce, TotalTorque, bVisLog, MassTrafficSubsystem.Get(), TEXT("Pg"));
...
)
{
	...
	for (int WheelIndex = 0; WheelIndex < SimplePhysicsVehicleFragment.VehicleSim.WheelSims.Num(); WheelIndex++)
	{
		...
		if (bVisLog)
		{
			UE_VLOG_ARROW(MassTrafficSubsystem.Get(), TEXT("MassTraffic Suspension"), Log, WorldSpaceX, T, FColor::Orange, TEXT("T"));
		}
		...
		if (Distance < MinLength)
		{
			if (bVisLog)
			{
				UE_VLOG_LOCATION(MassTrafficSubsystem.Get(), TEXT("MassTraffic Suspension"), Warning, WorldSpaceX + AxisWorld * 100.0f, 5.0f, FColor::Black, TEXT("Susp < Min (%0.2f < %0.2f)"), Distance, -PSuspension.Setup().SuspensionMaxRaise);
			}
		...
		if (bVisLog)
		{
			UE_VLOG_SEGMENT(MassTrafficSubsystem.Get(), TEXT("MassTraffic Suspension"), Log, WorldSpaceX + FVector(5, 10, 10), WorldSpaceX + FVector(15, 10, 10), FColor::Black, TEXT(""));
			UE_VLOG_ARROW(MassTrafficSubsystem.Get(), TEXT("MassTraffic Suspension"), Log, WorldSpaceX + FVector(10), WorldSpaceX + DX + FVector(10), FColor::Black, TEXT("DX"));
		}
