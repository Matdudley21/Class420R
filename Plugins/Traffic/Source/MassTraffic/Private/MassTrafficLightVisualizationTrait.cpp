...
void UMassTrafficLightVisualizationTrait::BuildTemplate(FMassEntityTemplateBuildContext& BuildContext, UWorld& World) const
{
	...
	const uint32 TrafficLightsParamsHash = UE::StructUtils::GetStructCrc32(FConstStructView::Make(RegisteredTrafficLightsParams));
	const FConstSharedStruct TrafficLightsParamsFragment = EntitySubsystem->GetOrCreateConstSharedFragment<FMassTrafficLightsParameters>(TrafficLightsParamsHash, RegisteredTrafficLightsParams);
