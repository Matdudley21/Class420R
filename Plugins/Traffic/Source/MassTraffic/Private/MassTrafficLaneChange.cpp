	...
	const UMassEntitySubsystem& EntitySubsystem)
{
	...
	{
		const FMassEntityView EntityView_Marching(EntitySubsystem, Entity_Marching);
	...
	const UObject* VisLogOwner)
{
...

	{
		...
		{
			const FMassEntityView EntityView_Current_Next(EntitySubsystem, Entity_Current_Next);
	...
	while (Entity_Marching.IsSet())
	{
		...
		{
			UE::MassTraffic::VisLogMalformedNextLaneLinks(EntitySubsystem, TrafficLaneData->LaneHandle.Index, TrafficLaneData->TailVehicle, Entity_Current, /*MarchEjectAt*/1000, VisLogOwner);
			return false;
		}
		...
		{
			UE_LOG(LogMassTraffic, Warning, TEXT("%s - March eject at %d"), ANSI_TO_TCHAR(__FUNCTION__), MarchCount);
			VisLogMalformedNextLaneLinks(EntitySubsystem, TrafficLaneData->LaneHandle.Index, TrafficLaneData->TailVehicle, Entity_Current, /*MarchEjectAt*/1000, VisLogOwner);
			return false;
		}
...
FMassEntityHandle FindNearestTailVehicleOnNextLanes(const FZoneGraphTrafficLaneData& CurrentTrafficLaneData, const FVector& VehiclePosition, const UMassEntitySubsystem& EntitySubsystem, const EMassTrafficFindNextLaneVehicleType Type)
{
	...
	auto TestAndSetNextVehicleEntity = [&](const FMassEntityHandle NextVehicleEntity) -> void
	{
		...
		const FMassEntityView NextVehicleEntityView(EntitySubsystem, NextVehicleEntity);
...
bool CheckNextVehicle(const FMassEntityHandle Entity, const FMassEntityHandle NextEntity, const UMassEntitySubsystem& EntitySubsystem)
{
	...
	const FMassEntityView NextEntityView(EntitySubsystem, NextEntity);
