...
void UMassTrafficVehicleUpdateCustomVisualizationProcessor::Execute(UMassEntitySubsystem& EntitySubsystem, FMassExecutionContext& Context)
{
				...
				switch (RepresentationFragment.CurrentRepresentation)
				{
					...
					case EMassRepresentationType::LowResSpawnedActor:
					{
						...
						if (Actor)
						{
							...
							Context.Defer().EmplaceCommand<FDeferredCommand>([Actor, NewActorTransform](UMassEntitySubsystem& System)
							{
							...
							if (!SimpleVehiclePhysicsFragments.IsEmpty())
							{
								...
								if (MassTrafficVehicleComponent)
								{
									...
									Context.Defer().EmplaceCommand<FDeferredCommand>([MassTrafficVehicleComponent, Entity](UMassEntitySubsystem& CallbackEntitySubsystem)
									{
		...
		DebugEntityQuery.ForEachEntityChunk(EntitySubsystem, Context, [this](FMassExecutionContext& Context)
			{
				...
				for (int EntityIdx = 0; EntityIdx < NumEntities; EntityIdx++)
				{
					...
					{
						UE_VLOG_LOCATION(MassTrafficSubsystem, TEXT("MassTraffic Vis"), Log, TransformFragment.GetTransform().GetLocation() + FVector(50.0f, 0.0f, 200.0f), /*Radius*/ 10.0f, UE::MassLOD::LODColors[CurrentVisualIdx], TEXT("%d %d"), CurrentVisualIdx, Context.GetEntity(EntityIdx).Index);
					}
				}
			});
	}
