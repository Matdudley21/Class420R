...
void FTrafficClientBubbleHandler::DebugValidateBubbleOnServer()
{
	...
	for (int32 OuterIdx = 0; OuterIdx < (*Agents).Num(); ++OuterIdx)
	{
		...
		FMassReplicatedAgentFragment& AgentFragment = EntitySystem->GetFragmentDataChecked<FMassReplicatedAgentFragment>(LookupData.Entity);

		if (AgentFragment.AgentsData[ClientHandle.GetIndex()].LastUpdateTime == World->GetRealTimeSeconds())
		{
			const FTransformFragment& FragmentTransform = EntitySystem->GetFragmentDataChecked<FTransformFragment>(LookupData.Entity);
			const FVector LocFragment = FragmentTransform.GetTransform().GetLocation();
			const FVector& AgentPos = OuterItem.Agent.GetPosition();
			...
			const TOptional<float> Yaw = UE::AI::GetYawFromQuaternion(FragmentTransform.GetTransform().GetRotation());
			check(Yaw.IsSet());
	...
	for (int32 Idx = 0; Idx < (*Agents).Num(); ++Idx)
	{
		...
		if (EntityInfo)
		{
			if (EntityInfo->ReplicationID == Item.ReplicationID)
			{
				...
				if (bIsEntityValid)
				{
					const FTransformFragment& FragmentTransform = EntitySystem->GetFragmentDataChecked<FTransformFragment>(EntityInfo->Entity);
					const FVector LocFragment = FragmentTransform.GetTransform().GetLocation();
					const FVector& AgentPos = Agent.GetPosition();
