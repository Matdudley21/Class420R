...
void UMassTrafficFindNextVehicleProcessor::Execute(UMassEntitySubsystem& EntitySubsystem, FMassExecutionContext& Context)
{
	// Gather all fragments
	TArray<FMassEntityHandle> AllVehicles;
	EntityQuery.ForEachEntityChunk(EntitySubsystem, Context, [&](const FMassExecutionContext& QueryContext)
	{
	...
	AllVehicles.Sort(
		[&EntitySubsystem](const FMassEntityHandle& EntityA, const FMassEntityHandle& EntityB)
		{
			const FMassZoneGraphLaneLocationFragment& A = EntitySubsystem.GetFragmentDataChecked<FMassZoneGraphLaneLocationFragment>(EntityA);
			const FMassZoneGraphLaneLocationFragment& B = EntitySubsystem.GetFragmentDataChecked<FMassZoneGraphLaneLocationFragment>(EntityB);
	...
	for (int32 Index = 0; Index < AllVehicles.Num() - 1; ++Index)
	{
		...
		FMassEntityView VehicleEntityView(EntitySubsystem, VehicleEntity);
		...
		const FMassZoneGraphLaneLocationFragment& NextLaneLocationFragment = EntitySubsystem.GetFragmentDataChecked<FMassZoneGraphLaneLocationFragment>(NextVehicleEntity);
	...
	const FMassEntityView LastVehicleEntityView(EntitySubsystem, LastVehicleEntity);
			...
			if (!NextVehicleFragment.HasNextVehicle())
			{
				if (const FZoneGraphTrafficLaneData* TrafficLaneData = MassTrafficSubsystem->GetTrafficLaneData(LaneLocationFragment.LaneHandle))
				{
					...
					for (const FZoneGraphTrafficLaneData* NextTrafficLaneData : TrafficLaneData->NextLanes)
					{
						...
						{
							const FMassZoneGraphLaneLocationFragment& TailVehicleLaneLocation = EntitySubsystem.GetFragmentDataChecked<FMassZoneGraphLaneLocationFragment>(NextTrafficLaneData->TailVehicle);
