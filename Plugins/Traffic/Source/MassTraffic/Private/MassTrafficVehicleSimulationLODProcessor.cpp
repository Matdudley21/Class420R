...
void UMassTrafficVehicleSimulationLODProcessor::Execute(UMassEntitySubsystem& EntitySubsystem, FMassExecutionContext& Context)
{
	check(LODSubsystem);
	...
	
	{
		...
		EntityQueryCalculateLOD.ForEachEntityChunk(EntitySubsystem, Context, [this](FMassExecutionContext& Context)
		{
...

	{
		...
		{
			EntityQueryAdjustDistances.ForEachEntityChunk(EntitySubsystem, Context, [this](FMassExecutionContext& QueryContext)
			{
...

	{
		...
		EntityQueryLODChange.ForEachEntityChunk(EntitySubsystem, Context, [this](FMassExecutionContext& QueryContext)
		{
			...
			for (int EntityIdx = 0; EntityIdx < NumEntities; EntityIdx++)
			{
				...
				if (SimulationLODFragment.LOD != SimulationLODFragment.PrevLOD)
				{
					...
					if (SimulationLODFragment.LOD <= EMassLOD::Medium) 
					{
						...
						{
							if (PhysicsSharedFragment.Template && PhysicsSharedFragment.Template->SimpleVehiclePhysicsFragmentTemplate.IsValid())
							{
								// Add FDataFragment_PIDVehicleControl & FDataFragment_SimpleVehiclePhysics fragments
								QueryContext.Defer().PushCommand(
									FMassCommandAddFragmentInstanceList(QueryContext.GetEntity(EntityIdx), {
										PhysicsSharedFragment.Template->SimpleVehiclePhysicsFragmentTemplate,
										FInstancedStruct::Make<FMassTrafficPIDVehicleControlFragment>(PhysicsSharedFragment.Template->SimpleVehiclePhysicsConfig.MaxSteeringAngle),
										FInstancedStruct::Make<FMassTrafficPIDControlInterpolationFragment>(),
										FInstancedStruct::Make<FMassTrafficVehicleDamageFragment>()
									})
								);
							}
						}
					...
					else if (SimulationLODFragment.PrevLOD <= EMassLOD::Medium)
					{
						...
						if (SimpleVehiclePhysicsFragments.Num())
						{
							...
							QueryContext.Defer().PushCommand(
								FCommandRemoveFragmentList(QueryContext.GetEntity(EntityIdx), {
									FMassTrafficVehiclePhysicsFragment::StaticStruct(),
									FMassTrafficPIDVehicleControlFragment::StaticStruct(),
									FMassTrafficPIDControlInterpolationFragment::StaticStruct(),
									FMassTrafficVehicleDamageFragment::StaticStruct()
								})
							);
						}
					}
...

	{
		...
		EntityQueryVariableTick.ForEachEntityChunk(EntitySubsystem, Context, [this, Time](FMassExecutionContext& QueryContext)
		{
	...
	if (GMassTrafficDebugSimulationLOD)
	{
		...
		EntityQuery.ForEachEntityChunk(EntitySubsystem, Context, [this](FMassExecutionContext& QueryContext)
		{
			...
			for (int EntityIdx = 0; EntityIdx < NumEntities; EntityIdx++)
			{
				...
				{
					UE_VLOG_LOCATION(MassTrafficSubsystem.Get(), TEXT("MassTraffic Simulation LOD"), Log, EntityLocation.GetTransform().GetLocation() + FVector(0.0f, 0.0f, 300.0f), /*Radius*/ 10.0f, UE::MassLOD::LODColors[SimulationLODIdx], TEXT("%d %s %d"), SimulationLODIdx, bShouldTickChunkThisFrame ? TEXT("") : TEXT("(x)"), QueryContext.GetEntity(EntityIdx).Index);
				}
			}
		});
	}
