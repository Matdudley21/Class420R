...
EMassActorSpawnRequestAction  UMassTrafficLightRepresentationActorManagement::OnPostActorSpawn(const FMassActorSpawnRequestHandle& SpawnRequestHandle, FConstStructView SpawnRequest, UMassEntitySubsystem* EntitySubsystem) const
{
	...
	const FMassEntityView IntersectionMassEntityView(*EntitySubsystem, MassActorSpawnRequest.MassAgent);
	...
	for (const FMassTrafficLight& TrafficLight : TrafficIntersectionFragment.TrafficLights)
	{
		...
		const FStaticMeshInstanceVisualizationMeshDesc& MeshDesc = ISMInfo[TrafficLightStaticMeshDescIndex].GetDesc().Meshes[0];
