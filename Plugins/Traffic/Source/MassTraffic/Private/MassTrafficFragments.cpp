	...
	UMassEntitySubsystem& EntitySubsystem)
{
	...
	FMassEntityView EntityView_Behind(EntitySubsystem, InVehicleEntity_Behind);
	...
	UMassEntitySubsystem& EntitySubsystem)
{
	...
	{
		UE_LOG(LogMassTraffic, Error, TEXT("%s - Could not begin lane change progression! Bad lane change state requested (%d)."), ANSI_TO_TCHAR(__FUNCTION__), InLaneChangeSide);
		return false;
	}
	...
	{
		const FMassEntityView EntityView_Behind(EntitySubsystem, VehicleEntity_Initial_Behind);
		FMassTrafficNextVehicleFragment& NextVehicleFragment_Behind = EntityView_Behind.GetFragmentData<FMassTrafficNextVehicleFragment>();
		NextVehicleFragment_Behind.AddLaneChangeNextVehicle(VehicleEntity_Current);
	}
...

	{
		...
		{
			const FMassZoneGraphLaneLocationFragment& LaneLocationFragment_LaneChangeGhost_Initial = EntitySubsystem.GetFragmentDataChecked<FMassZoneGraphLaneLocationFragment>(TrafficLaneData_Initial->GhostTailVehicle_FromLaneChangingVehicle);
...
)
{
	...
	if (IsLaneChangeInProgress())
	{
		...
		{
			EndLaneChangeProgression(VehicleLightsFragment_Current, NextVehicleFragment_Current, EntitySubsystem);
	...
	const UMassEntitySubsystem& EntitySubsystem)
{
	...
	if (VehicleEntity_Initial_Behind.IsSet())
	{
		FMassEntityView EntityView_Other(EntitySubsystem, VehicleEntity_Initial_Behind);
	...
	if (!OtherVehicleEntities_Behind.IsEmpty())
	{
		for (FMassEntityHandle OtherVehicleEntity : OtherVehicleEntities_Behind)
		{
			...
			FMassEntityView EntityView_Other(EntitySubsystem, OtherVehicleEntity);
			FMassTrafficNextVehicleFragment& NextVehicleFragment_Other = EntityView_Other.GetFragmentData<FMassTrafficNextVehicleFragment>();
