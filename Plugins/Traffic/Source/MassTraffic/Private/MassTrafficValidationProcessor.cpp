...
void UMassTrafficValidationProcessor::Execute(UMassEntitySubsystem& EntitySubsystem, FMassExecutionContext& Context)
{
	...
	for (const FMassTrafficZoneGraphData& TrafficZoneData : MassTrafficSubsystem->GetTrafficZoneGraphData())
	{
		...
		for (const FZoneGraphTrafficLaneData& TrafficLaneData : TrafficZoneData.TrafficLaneDataArray)
		{
			...
			{
				FMassZoneGraphLaneLocationFragment& TailVehicleLaneLocationFragment = EntitySubsystem.GetFragmentDataChecked<FMassZoneGraphLaneLocationFragment>(TrafficLaneData.TailVehicle);
				...
				if (!ensure(TailVehicleLaneLocationFragment.LaneHandle == TrafficLaneData.LaneHandle))
				{
					...
					UE::ZoneGraph::Query::CalculateLocationAlongLane(*ZoneGraphStorage, TailVehicleLaneLocationFragment.LaneHandle, TailVehicleLaneLocationFragment.DistanceAlongLane, TailVehicleLaneLocation);
						...
						UE_VLOG_LOCATION(MassTrafficSubsystem.Get(), TEXT("MassTraffic Validation"), Warning, LaneBeginPoint, 10.0f, FColor::Red, TEXT("%s tail vehicle (%d) is on different lane %d"), *TrafficLaneData.LaneHandle.ToString(), TrafficLaneData.TailVehicle.Index, TailVehicleLaneLocationFragment.LaneHandle.Index);
						UE_VLOG_SEGMENT_THICK(MassTrafficSubsystem.Get(), TEXT("MassTraffic Validation"), Warning, LaneBeginPoint, TailVehicleLaneLocation.Position, FColor::Red, 5.0f, TEXT(""));
					#endif
				}
			}
			...
			if (TrafficLaneData.SpaceAvailable < TrafficLaneData.Length - 1.0f && !TrafficLaneData.TailVehicle.IsSet())
			{
				...
				#if ENABLE_VISUAL_LOG
					UE_VLOG_LOCATION(MassTrafficSubsystem.Get(), TEXT("MassTraffic Validation"), Warning, LaneMidPoint, 10.0f, FColor::Red, TEXT("%s is empty but doesn't have full space available (Available: %0.2f  Length: %0.2f)"), *TrafficLaneData.LaneHandle.ToString(), TrafficLaneData.SpaceAvailable, TrafficLaneData.Length);
				#endif
			}
	...
	// Vehicle validation
	EntityQuery_Conditional.ForEachEntityChunk(EntitySubsystem, Context, [&](FMassExecutionContext& ComponentSystemExecutionContext)
		{
			...
			for (int32 Index = 0; Index < NumEntities; ++Index)
			{
				...
				if (Actor && (RepresentationFragment.CurrentRepresentation == EMassRepresentationType::LowResSpawnedActor || RepresentationFragment.CurrentRepresentation == EMassRepresentationType::HighResSpawnedActor))
				{
					...
					{
						UE_VLOG_LOCATION(MassTrafficSubsystem.Get(), TEXT("MassTraffic Validation"), Warning, LaneLocationTransform.GetLocation(), 10.0f, FColor::Orange, TEXT("%d actor deviated from lane"), VehicleEntity.Index);
						UE_VLOG_LOCATION(MassTrafficSubsystem.Get(), TEXT("MassTraffic Validation"), Warning, TransformFragment.GetTransform().GetLocation(), 10.0f, FColor::Blue, TEXT("%d"), VehicleEntity.Index);
						UE_VLOG_SEGMENT_THICK(MassTrafficSubsystem.Get(), TEXT("MassTraffic Validation"), Warning, LaneLocationTransform.GetLocation(), Actor->GetActorLocation(), FColor::Orange, 5.0f, TEXT(""));
					}
				}
				else
				{
					...
					if (!ensure(VehicleDeviationDistance < VehicleMajorDeviationDistanceThreshold) ||
						(!LaneChangeFragment.IsLaneChangeInProgress() && !ensure(VehicleDeviationDistance < VehicleDeviationDistanceThreshold)))
					{
						UE_VLOG_LOCATION(MassTrafficSubsystem.Get(), TEXT("MassTraffic Validation"), Warning, LaneLocationTransform.GetLocation(), 10.0f, FColor::Orange, TEXT("%d deviated from lane"), VehicleEntity.Index);
						UE_VLOG_SEGMENT_THICK(MassTrafficSubsystem.Get(), TEXT("MassTraffic Validation"), Warning, LaneLocationTransform.GetLocation(), TransformFragment.GetTransform().GetLocation(), FColor::Orange, 5.0f, TEXT(""));
					}
				}
				...
				{
					UE_VLOG_LOCATION(MassTrafficSubsystem.Get(), TEXT("MassTraffic Validation"), Error, LaneLocationTransform.GetLocation(), 10.0f, FColor::Red, TEXT("%d lane location distance (%f) is outside the lane range (0 to %f)"), VehicleEntity.Index, LaneLocationFragment.DistanceAlongLane, LaneLocationFragment.LaneLength);
				}

				// Check speed
				if (!ensure(VehicleControlFragment.Speed < VehicleMaxSpeed))
				{
					UE_VLOG_LOCATION(MassTrafficSubsystem.Get(), TEXT("MassTraffic Validation"), Error, TransformFragment.GetTransform().GetLocation(), 10.0f, FColor::Red, TEXT("%d speed (%0.2f) exceeds VehicleMaxSpeed (%0.2f)"), VehicleEntity.Index, VehicleControlFragment.Speed, VehicleMaxSpeed);
				}
				...
				if (SimulationLODFragment.LOD >= EMassLOD::Off && SimulationLODFragment.PrevLOD >= EMassLOD::Off)
				{
					...
					{
						UE_VLOG_LOCATION(MassTrafficSubsystem.Get(), TEXT("MassTraffic Validation"), Warning, TransformFragment.GetTransform().GetLocation(), 10.0f, FColor::Red, TEXT("%d shouldn't be drawn"), VehicleEntity.Index);
					}
				}
				...
				if (NextVehicleFragment.HasNextVehicle())
				{
					...
					{
						UE_VLOG_LOCATION(MassTrafficSubsystem.Get(), TEXT("MassTraffic Validation"), Error, TransformFragment.GetTransform().GetLocation(), 10.0f, FColor::Red, TEXT("%d's NextVehicle is itself"), VehicleEntity.Index);
					}
					else
					{
						// Make sure we don't go past our next vehicle
						FMassEntityView NextVehicleEntityView(EntitySubsystem, NextVehicleFragment.GetNextVehicle());
						...
						if (LaneLocationFragment.LaneHandle == NextVehicleLaneLocationFragment.LaneHandle)
						{
								...
								!NextVehicleLaneChangeFragment.IsLaneChangeInProgress())
							{
								...
								UE_VLOG_LOCATION(MassTrafficSubsystem.Get(), TEXT("MassTraffic Validation"), Error, TransformFragment.GetTransform().GetLocation(), RadiusFragment.Radius, FColor::Red, TEXT("%d @ %0.2f is further along the lane than it's next vehicle %d @ %0.2f (Sim LOD %d)"), VehicleEntity.Index, LaneLocationFragment.DistanceAlongLane, NextVehicleFragment.GetNextVehicle().Index, NextVehicleLaneLocationFragment.DistanceAlongLane, SimulationLODFragment.LOD.GetValue());
								UE_VLOG_SEGMENT(MassTrafficSubsystem.Get(), TEXT("MassTraffic Validation"), Error, TransformFragment.GetTransform().GetLocation() + FVector(0,0,100), NextVehicleTransformFragment.GetTransform().GetLocation() + FVector(0,0,100), FColor::Red, TEXT("%0.2f"), AvoidanceFragment.DistanceToNext);
								UE_VLOG_LOCATION(MassTrafficSubsystem.Get(), TEXT("MassTraffic Validation"), Error, NextVehicleTransformFragment.GetTransform().GetLocation(), NextVehicleRadiusFragment.Radius, FColor::White, TEXT(""));
								UE_VLOG_LOCATION(MassTrafficSubsystem.Get(), TEXT("MassTraffic Validation"), Error, RawLaneLocation.Position, 10.0f, FColor::Red, TEXT(""));
								UE_VLOG_LOCATION(MassTrafficSubsystem.Get(), TEXT("MassTraffic Validation"), Error, NextVehicleRawLaneLocation.Position, 10.0f, FColor::White, TEXT(""));
							}
						}
					}
					...
					{
						CheckNextVehicle(VehicleEntity, NextVehicleFragment.GetNextVehicle(), EntitySubsystem);
					}
				}
