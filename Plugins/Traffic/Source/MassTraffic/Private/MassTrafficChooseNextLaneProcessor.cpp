...
void UMassTrafficChooseNextLaneProcessor::Execute(UMassEntitySubsystem& EntitySubsystem, FMassExecutionContext& Context)
{
	...
	// Advance agents
	EntityQuery_Conditional.ForEachEntityChunk(EntitySubsystem, Context, [&](FMassExecutionContext& QueryContext)
	{	
		...
		for (int32 Index = 0; Index < NumEntities; ++Index)
		{
			...
			if (GMassTrafficDebugChooseNextLane)
			{
				...
				const FTransformFragment& TransformFragment = EntitySubsystem.GetFragmentDataChecked<FTransformFragment>(Entity);
			...
			if (CurrentLane.NextLanes.IsEmpty())
			{
				...
				#if WITH_MASSTRAFFIC_DEBUG
					UE_VLOG_LOCATION(MassTrafficSubsystem.Get(), TEXT("MassTraffic Validation"), Error,
			...
			// VisLog
				...
				if (OptionalDebugFragments[Index].bVisLog)
				{
					if (VehicleControlFragment.NextLane)
					{
						...
						UE_VLOG_ARROW(MassTrafficSubsystem.Get(), TEXT("MassTraffic NextLane"), Display,
					...
					{
						UE_VLOG_LOCATION(MassTrafficSubsystem.Get(), TEXT("MassTraffic NextLane"), Error,
