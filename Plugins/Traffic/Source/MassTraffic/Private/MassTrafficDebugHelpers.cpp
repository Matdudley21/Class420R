...
void VisLogMalformedNextLaneLinks(const UMassEntitySubsystem& EntitySubsystem, int32 LaneIndex, const FMassEntityHandle TailVehicle, const FMassEntityHandle UndiscoveredVehicle, int32 MarchEjectAt, const UObject* VisLogOwner)
{
	...
	const FMassEntityView TailVehicleEntityView(EntitySubsystem, TailVehicle);
	...
	{
		const FMassEntityView UndiscoveredVehicleEntityView(EntitySubsystem, UndiscoveredVehicle);
	...
	while (MarchingVehicleEntityView.IsSet())
	{
		...
		// Log line to next
		FMassEntityView NextVehicleEntityView(EntitySubsystem, NextVehicle);
