...
void UMassTrafficUpdateDistanceToNearestObstacleProcessor::Execute(UMassEntitySubsystem& EntitySubsystem, FMassExecutionContext& Context)
{
	// Process fragments
	EntityQuery_Conditional.ForEachEntityChunk(EntitySubsystem, Context, [this, &EntitySubsystem](FMassExecutionContext& QueryContext)
		{
			...
			for (int32 Index = 0; Index < NumEntities; ++Index)
			{
					...
					const FAgentRadiusFragment& NextRadiusFragment) -> void
				{
					...
					// Debug
					UE::MassTraffic::DrawDebugDistanceToNext(GetWorld(), CurrentLocation, NextLocation, AvoidanceFragment.DistanceToNext, CombineDistanceToNextType, bVisLog, MassTrafficSubsystem.Get());
				...
				{
					FMassEntityView NextView(EntitySubsystem, NextVehicleFragment.GetNextVehicle());
				...
				for (FMassEntityHandle NextVehicle_LaneChange : NextVehicleFragment.NextVehicles_LaneChange)
				{
					...
					const FMassEntityView NextView(EntitySubsystem, NextVehicle_LaneChange);
				...
				{
					const FMassEntityView NextView(EntitySubsystem, NextVehicleFragment.NextVehicle_SplittingLaneGhost);
					const FMassTrafficVehicleSimulationParameters& NextSimulationParams = NextView.GetConstSharedFragmentData<FMassTrafficVehicleSimulationParameters>();
					const FTransformFragment& NextTransformFragment = NextView.GetFragmentData<FTransformFragment>();
					const FAgentRadiusFragment& NextRadiusFragment = NextView.GetFragmentData<FAgentRadiusFragment>();
					const FMassTrafficVehicleLaneChangeFragment& NextLaneChangeFragment = NextView.GetFragmentData<FMassTrafficVehicleLaneChangeFragment>();
				...
				{
					const FMassEntityView NextView(EntitySubsystem, NextVehicleFragment.NextVehicle_MergingLaneGhost);
					const FTransformFragment& NextTransformFragment = NextView.GetFragmentData<FTransformFragment>();
					const FAgentRadiusFragment& NextRadiusFragment = NextView.GetFragmentData<FAgentRadiusFragment>();
			...
			if (!OptionalObstacleListFragments.IsEmpty())
			{
				for (int32 Index = 0; Index < NumEntities; ++Index)
				{
					...
					for (const FMassEntityHandle& Obstacle : OptionalObstacleListFragment.Obstacles) 
					{
						...
						if (EntitySubsystem.IsEntityValid(Obstacle) && Obstacle.IsSet())
						{
							FMassEntityView ObstacleView(EntitySubsystem, Obstacle);
							...
							{
								UE_VLOG_ARROW(MassTrafficSubsystem.Get(), LogMassTraffic, Display, TransformFragment.GetTransform().GetLocation(), ObstacleTransformFragment.GetTransform().GetLocation(), FColor::Yellow, TEXT(""));						
							}
							#endif
						}
					}
