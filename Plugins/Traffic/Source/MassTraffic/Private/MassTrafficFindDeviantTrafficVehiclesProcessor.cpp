...
void UMassTrafficFindDeviantTrafficVehiclesProcessor::Initialize(UObject& Owner)
{
...
static void RemoveDeviantFragments(const UMassEntitySubsystem& EntitySubsystem, const FMassExecutionContext& Context, UMassNavigationSubsystem& MovementSubsystem, const int32 Index)
{
	...
	Context.Defer().PushCommand(FCommandRemoveFragmentList(Entity,
		{FMassNavigationObstacleGridCellLocationFragment::StaticStruct(),	// Not an avoidance obstacle anymore
		FMassCrowdObstacleFragment::StaticStruct(),						// Not a zone graph dynamic obstacle anymore
		FMassAvoidanceColliderFragment::StaticStruct()}));
}

void UMassTrafficFindDeviantTrafficVehiclesProcessor::Execute(UMassEntitySubsystem& EntitySubsystem, FMassExecutionContext& Context)
{
	// Look for deviant vehicles
	NominalTrafficVehicleEntityQuery.ForEachEntityChunk(EntitySubsystem, Context, [&](FMassExecutionContext& QueryContext)
	{
		...
		for (int32 Index = 0; Index < NumEntities; ++Index)
		{
			...
			if (Actor != nullptr && RepresentationFragment.CurrentRepresentation == EMassRepresentationType::HighResSpawnedActor)
			{
				...
				if (Deviation > VehicleDeviationTolerance)
				{
					...
					QueryContext.Defer().PushCommand(FCommandAddFragmentList(Entity,
						{FMassNavigationObstacleGridCellLocationFragment::StaticStruct(),	// Needed to become an avoidance obstacle
					...
					QueryContext.Defer().PushCommand(FCommandAddFragmentInstance(Entity, FInstancedStruct::Make(ColliderFragment)));
					...
					UE_VLOG_SEGMENT_THICK(MassTrafficSubsystem.Get(), TEXT("MassTraffic Deviants"), Log, ActorLocation, LaneLocationTransform.GetLocation(), FColor::Red, 3.0f, TEXT(""));
				}
			}
		}
	});
	...
	DeviantTrafficVehicleEntityQuery.ForEachEntityChunk(EntitySubsystem, Context, [&](FMassExecutionContext& QueryContext)
	{
		const TConstArrayView<FMassZoneGraphLaneLocationFragment> ZoneGraphLaneLocationFragments = QueryContext.GetFragmentView<FMassZoneGraphLaneLocationFragment>();
		const TConstArrayView<FMassTrafficLaneOffsetFragment> LaneOffsetFragments = QueryContext.GetFragmentView<FMassTrafficLaneOffsetFragment>();
		const TConstArrayView<FMassRepresentationFragment> RepresentationFragments = QueryContext.GetFragmentView<FMassRepresentationFragment>();
		...
		for (int32 Index = 0; Index < NumEntities; ++Index)
		{
			...
			if (Actor != nullptr && RepresentationFragment.CurrentRepresentation == EMassRepresentationType::HighResSpawnedActor)
			{
				...
				{
					// Debug
					UE_VLOG_LOCATION(MassTrafficSubsystem.Get(), TEXT("MassTraffic Deviants"), Log, ActorLocation, 10.0f, FColor::Green, TEXT("%d Corrected"), QueryContext.GetEntity(Index).Index);
				}
			}
			else
			{
				// Debug
				UE_VLOG(MassTrafficSubsystem.Get(), TEXT("MassTraffic Deviants"), Log, TEXT("%d Corrected"), QueryContext.GetEntity(Index).Index);
			}

			if (!bDeviant)
			{
				RemoveDeviantFragments(EntitySubsystem, QueryContext, *NavigationSubsystem, Index);
			}
		}
	});

	// Remove obstacle fragment from implicitly corrected vehicles
	CorrectedTrafficVehicleEntityQuery.ForEachEntityChunk(EntitySubsystem, Context, [&](const FMassExecutionContext& QueryContext)
	{
		const int32 NumEntities = QueryContext.GetNumEntities();
		for (int32 Index = 0; Index < NumEntities; ++Index)
		{
