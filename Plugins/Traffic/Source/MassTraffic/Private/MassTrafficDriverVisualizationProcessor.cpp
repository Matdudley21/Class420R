...
void UMassTrafficDriverVisualizationProcessor::Execute(UMassEntitySubsystem& EntitySubsystem, FMassExecutionContext& Context)
{
	// Skip driver vis?
	...
	EntityQuery_Conditional.ForEachEntityChunk(EntitySubsystem, Context, [&, this](FMassExecutionContext& QueryContext)
	{
		...
		for (int32 EntityIdx = 0; EntityIdx < NumEntities; EntityIdx++)
		{
			...
			if (RepresentationLODFragment.LOD <= GMassTrafficMaxDriverVisualizationLOD && ViewerInfoFragment.ClosestViewerDistanceSq <= MaxDriverVisualizationDistanceSq && DriverStaticMeshDescIndex != INDEX_NONE)
			{
				...
				if (const UAnimToTextureDataAsset* AnimData = DriverType.AnimationData.Get())
				{
					...
					FAnimToTextureInstancePlaybackData CustomData;
					...
					if (bRemoveDriver)
					{
						...
						{
							QueryContext.Defer().EmplaceCommand<FDeferredCommand>([=](UMassEntitySubsystem&)
							{
								if (IsValid(Actor))
								{
...
bool UMassTrafficDriverVisualizationProcessor::PopulateAnimEvalFromAnimState(
	...
	FAnimToTextureInstancePlaybackData& OutPlaybackData)
{
	...
	FAnimToTextureInstancePlaybackData& OutPlaybackData)
{
	...
	if (AnimData && AnimData->Animations.IsValidIndex(VariationAnimStateIndex))
	{
		...
		const FAnimInfo& AnimInfo = AnimData->Animations[VariationAnimStateIndex];
		OutPlaybackData.CurrentState.StartFrame = AnimInfo.AnimStart;
		OutPlaybackData.CurrentState.NumFrames = AnimInfo.NumFrames;
		OutPlaybackData.CurrentState.bLooping = AnimInfo.bLooping;
