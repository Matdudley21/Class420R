...
void FMassTrafficFieldOperationContext::ForEachTrafficVehicle(FTrafficVehicleOnLaneExecuteFunction ExecuteFunction) const
{
	...
	{		
		...
		{
			TrafficLaneData->ForEachVehicleOnLane(EntitySubsystem, [&](const FMassEntityView& VehicleEntityView, FMassTrafficNextVehicleFragment& NextVehicleFragment, FMassZoneGraphLaneLocationFragment& LaneLocationFragment)
			{
...
void FMassTrafficFieldOperationContext::ForEachTrafficIntersection(FTrafficIntersectionExecuteFunction ExecuteFunction) const
{
	...
	{
		FMassTrafficIntersectionFragment& TrafficIntersectionFragment = EntitySubsystem.GetFragmentDataChecked<FMassTrafficIntersectionFragment>(TrafficIntersectionEntity);
...
void UMassTrafficFieldOperationsProcessorBase::Execute(UMassEntitySubsystem& EntitySubsystem, FMassExecutionContext& Context)
{
	if (UMassTrafficSubsystem* TrafficSubsystem = MassTrafficSubsystem.Get())
	{
...
void UMassTrafficForceTrafficVehicleViewerLODFieldOperation::Execute(FMassTrafficFieldOperationContext& Context)
{
	...
	if (GMassTrafficDebugViewerLOD)
	{
		...
		Context.ForEachTrafficVehicle([this, &Context](FZoneGraphTrafficLaneData& TrafficLaneData, const FMassEntityView& VehicleEntityView, struct FMassTrafficNextVehicleFragment& NextVehicleFragment, struct FMassZoneGraphLaneLocationFragment& LaneLocationFragment)
		{
			...
			DrawDebugLine(Context.EntitySubsystem.GetWorld(), TransformFragment.GetTransform().GetLocation() + FVector(-25.0f, -25.0f, 200.0f), TransformFragment.GetTransform().GetLocation() + FVector(25.0f, 25.0f, 200.0f), FColor::Red, false, -1, 0, 5.0f);

			// Draw a point above to show the override LOD  
			DrawDebugPoint(Context.EntitySubsystem.GetWorld(), TransformFragment.GetTransform().GetLocation() + FVector(0.0f, 0.0f, 250.0f), 10.0f, UE::MassLOD::LODColors[VisualizationLODFragment.LOD]);
...
void UMassTrafficFrameStartFieldOperationsProcessor::Execute(UMassEntitySubsystem& EntitySubsystem, FMassExecutionContext& Context)
{
	// Re-cache lanes & intersections for Movable traffic fields
	{
		TRACE_CPUPROFILER_EVENT_SCOPE(TEXT("UpdateMovableTrafficFields"))
		
		const TArray<UMassTrafficFieldComponent*>& Fields = MassTrafficSubsystem->GetFields();
		for (UMassTrafficFieldComponent* Field : Fields)
		{
			if (Field->Mobility == EComponentMobility::Movable && Field->bEnabled)
			{
				Field->UpdateOverlappedLanes(MassTrafficSubsystem.Get());
				Field->UpdateOverlappedIntersections(MassTrafficSubsystem.Get());
			}
		}
	}
