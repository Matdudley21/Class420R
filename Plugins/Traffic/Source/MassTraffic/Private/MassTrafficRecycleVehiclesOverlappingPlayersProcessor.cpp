...
void UMassTrafficRecycleVehiclesOverlappingPlayersProcessor::Execute(UMassEntitySubsystem& EntitySubsystem, FMassExecutionContext& Context)
{
			...
			if (bIsOverlappingPlayer)
			{
				...
				else
				{
					...
					Context.Defer().PushCommand(FCommandSwapTags(Entity,
						FMassTrafficVehicleTag::StaticStruct(),
						FMassTrafficRecyclableVehicleTag::StaticStruct()));
				}
			}
		}
	});
