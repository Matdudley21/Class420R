...
void UMassTrafficFindObstaclesProcessor::Execute(UMassEntitySubsystem& EntitySubsystem, FMassExecutionContext& Context)
{
	{
		// Reset obstacle lists
		TRACE_CPUPROFILER_EVENT_SCOPE(TEXT("ResetObstacleLists"))
		
		ObstacleAvoidingEntityQuery.ForEachEntityChunk(EntitySubsystem, Context, [&](FMassExecutionContext& QueryContext)
		{
...

	{
		...
		ObstacleEntityQuery.ForEachEntityChunk(EntitySubsystem, Context, [&](FMassExecutionContext& QueryContext)
		{
			...
			for (int32 Index = 0; Index < NumEntities; ++Index)
			{
				...
				// Debug draw obstacle
					...
					if (GMassTrafficDebugObstacleAvoidance)
					{
						...
						{
							UE_VLOG_LOCATION(MassTrafficSubsystem.Get(), TEXT("MassTraffic Avoidance"), Log, TransformFragment.GetTransform().GetLocation(), AgentRadiusFragment.Radius, FColor::Yellow, TEXT("%d Obstacle"), ObstacleEntity.Index);
						}
					}
				#endif
				...
				for (const FZoneGraphLaneHandle NearbyLane : NearbyLanes)
				{
					...
					if (NearestLocationOnLane.IsValid())
					{
						// Debug draw nearby lanes
							...
							if (GMassTrafficDebugObstacleAvoidance > 1)
							{
								UE_VLOG_LOCATION(MassTrafficSubsystem.Get(), TEXT("MassTraffic Avoidance"), Log, NearestLocationOnLane.Position, 10.0f, FColor::Magenta, TEXT("%d Nearby Lane"), ObstacleEntity.Index);
							}
						#endif
						...
						UE::MassTraffic::FindNearestVehiclesInLane(EntitySubsystem, *NearbyTrafficLane, NearestLocationOnLane.DistanceAlongLane, PreviousVehicle, NextVehicle);
						...
						if (PreviousVehicle.IsSet() && PreviousVehicle != ObstacleEntity)
						{
							// Debug draw line from avoiding vehicle -> obstacle
								...
								if (GMassTrafficDebugObstacleAvoidance)
								{
									FMassEntityView PreviousVehicleEntityView(EntitySubsystem, PreviousVehicle);
									...
									if (GMassTrafficDebugObstacleAvoidance > 1)
									{
										UE_VLOG_SEGMENT_THICK(MassTrafficSubsystem.Get(), TEXT("MassTraffic Avoidance"), Log, AvoidingVehicleLocation, TransformFragment.GetTransform().GetLocation(), FColor::Yellow, 5.0f, TEXT("%d Avoiding %d"), PreviousVehicle.Index, ObstacleEntity.Index);
							...
							FMassTrafficObstacleListFragment* ExistingObstacleListFragment = EntitySubsystem.GetFragmentDataPtr<FMassTrafficObstacleListFragment>(PreviousVehicle);
							...
							{
								// We can't use Context.Defer().PushCommand(FCommandAddFragmentInstance) here as we
								// might find multiple obstacles for a single vehicle this frame which would result in
								// multiple FCommandAddFragmentInstance to be queued. So instead we collect all the
		...
		for (const auto& VehicleToObstacles : ObstacleListsToAdd)
		{
			...
			Context.Defer().PushCommand(FCommandAddFragmentInstance(VehicleToObstacles.Key, FStructView::Make(NewObstacleListFragment)));
		}
	}
}