...
void UMassTrafficDriverVisualizationTrait::BuildTemplate(FMassEntityTemplateBuildContext& BuildContext, UWorld& World) const 
{
	...
	if (IsValid(RegisteredParams.DriverTypesData))
	{
		for (const FMassTrafficDriverTypeData& DriverType : RegisteredParams.DriverTypesData->DriverTypes)
		{
			...
			{
				StaticMeshInstanceVisualizationDesc.Meshes.Add(FStaticMeshInstanceVisualizationMeshDesc({DriverMesh.StaticMesh, DriverMesh.MaterialOverrides, DriverMesh.MinLODSignificance, DriverMesh.MaxLODSignificance, false/*bCastShadow*/, EComponentMobility::Movable}));
			}
	...
	const uint32 ParamsHash = UE::StructUtils::GetStructCrc32(FConstStructView::Make(RegisteredParams));
	const FConstSharedStruct ParamsSharedFragment = EntitySubsystem->GetOrCreateConstSharedFragment<FMassTrafficDriversParameters>(ParamsHash, RegisteredParams);
...
void UMassTrafficDriverInitializer::Execute(UMassEntitySubsystem& EntitySubsystem,
