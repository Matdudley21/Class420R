...
void UMassTrafficActorVehiclePhysicsProcessor::Execute(UMassEntitySubsystem& EntitySubsystem, FMassExecutionContext& Context)
{
	// Pass new vehicle control inputs to HighRes actors 
	ChaosPhysicsVehiclesQuery.ForEachEntityChunk(EntitySubsystem, Context, [&](FMassExecutionContext& ComponentSystemExecutionContext)
	{
		...
		for (int32 Index = 0; Index < NumEntities; ++Index)
		{
			...
			if (Actor != nullptr && Actor->Implements<UMassTrafficVehicleControlInterface>())
			{
				...
				{
					Context.Defer().EmplaceCommand<FDeferredCommand>([Actor, PIDVehicleControlFragment](UMassEntitySubsystem&)
					{
