...
void UMassTrafficFieldComponent::UpdateOverlappedIntersections(UMassTrafficSubsystem* MassTrafficSubsystem)
{
	TRACE_CPUPROFILER_EVENT_SCOPE(TEXT("MassTrafficFieldComponent Find Overlapped Lanes"))
	...
	for (const FZoneGraphTrafficLaneData* TrafficLaneData : TrafficLanes)
	{
		...
		if (TrafficLaneData->ConstData.bIsIntersectionLane)
		{
			...
			FMassEntityHandle TrafficIntersectionEntity = MassTrafficSubsystem->GetTrafficIntersectionEntity(LaneData.ZoneIndex);
