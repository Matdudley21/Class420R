...
EMassActorSpawnRequestAction  UMassTrafficTrailerRepresentationActorManagement::OnPostActorSpawn(const FMassActorSpawnRequestHandle& SpawnRequestHandle, FConstStructView SpawnRequest, UMassEntitySubsystem* EntitySubsystem) const
{
	...
	FMassEntityView TrailerMassEntityView(*EntitySubsystem, MassActorSpawnRequest.MassAgent);
	...
	if (AWheeledVehiclePawn* TrailerPawn = Cast<AWheeledVehiclePawn>(MassActorSpawnRequest.SpawnedActor))
	{
		...
		{
			const FMassEntityView VehicleMassEntityView(*EntitySubsystem, VehicleConstraintFragment.Vehicle);
