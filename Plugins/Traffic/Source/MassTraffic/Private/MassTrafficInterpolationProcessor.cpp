...
void UMassTrafficInterpolationProcessor::Execute(UMassEntitySubsystem& EntitySubsystem, FMassExecutionContext& Context)
{
	UWorld* World = GetWorld();
	
	EntityQueryNonOffLOD_Conditional.ForEachEntityChunk(EntitySubsystem, Context, [&](FMassExecutionContext& QueryContext)
	{
		// Get fragment lists
		...
		for (int32 Index = 0; Index < NumEntities; ++Index)
		{
			...
			// Debug
			UE::MassTraffic::DrawDebugInterpolatedAxles(GetWorld(), FrontAxleTransform.GetLocation(), RearAxleTransform.GetLocation(), bVisLog, MassTrafficSubsystem.Get());
			...
			UE::MassTraffic::AdjustVehicleTransformDuringLaneChange(LaneChangeFragment, ZoneGraphLaneLocationFragment.DistanceAlongLane, TransformFragment.GetMutableTransform(), World, bVisLog, MassTrafficSubsystem.Get());

			// Debug
			UE::MassTraffic::DrawDebugLaneSegment(GetWorld(), VehicleMovementInterpolationFragment.LaneLocationLaneSegment, bVisLog, MassTrafficSubsystem.Get());
		}
	});

	EntityQueryOffLOD_Conditional.ForEachEntityChunk(EntitySubsystem, Context, [&](FMassExecutionContext& QueryContext)
	{
		// Get fragment lists
		const int32 NumEntities = Context.GetNumEntities();
		const TConstArrayView<FMassZoneGraphLaneLocationFragment> LaneLocationFragments = QueryContext.GetFragmentView<FMassZoneGraphLaneLocationFragment>();
		...
		for (int32 Index = 0; Index < NumEntities; ++Index)
		{
			...
			UE::MassTraffic::AdjustVehicleTransformDuringLaneChange(LaneChangeFragment, ZoneGraphLaneLocationFragment.DistanceAlongLane
				, TransformFragment.GetMutableTransform(), World, bVisLog, MassTrafficSubsystem.Get());
