...
USTRUCT(BlueprintType)
struct MASSTRAFFIC_API FMassTrafficPeriod
{
	GENERATED_BODY()
	...
	bool AddTrafficLightControl(const int8 TrafficLightIndex, const EMassTrafficLightStateFlags TrafficLightStateFlags);

	FMassTrafficLightControl* GetTrafficLightControl(const int8 TrafficLightIndex); 
	...
	FORCEINLINE bool VehicleLaneClosesInNextPeriod(FZoneGraphTrafficLaneData* VehicleLane) const
	{
		...
		if (RealIndex == INDEX_NONE)
		{
			UE_LOG(LogMassTraffic, Error, TEXT("%s - For testing if 'open vehicle lane closes in next period', lane fragment was not found in 'open vehicle lanes'."),
				ANSI_TO_TCHAR(__FUNCTION__), RealIndex);
			return false;
		}
		if (RealIndex > 255)
		{
...
USTRUCT()
struct MASSTRAFFIC_API FMassTrafficNextVehicleFragment : public FMassFragment
{
	GENERATED_BODY()
	...
	FORCEINLINE bool AddLaneChangeNextVehicle(const FMassEntityHandle Entity_Current)
	{
		if (NextVehicles_LaneChange.IsFull())
		{
			UE_LOG(LogMassTraffic, Error, TEXT("%s - NextVehicles_LaneChange - Too full (at %d) to add to."),
				ANSI_TO_TCHAR(__FUNCTION__), NextVehicles_LaneChange.Max());
			return false;
		}
...
struct MASSTRAFFIC_API FMassTrafficVehicleLaneChangeFragment : public FMassFragment
{
	...
	bool AddOtherLaneChangeNextVehicle_ForVehicleBehind(FMassEntityHandle InVehicleEntity_Behind, UMassEntitySubsystem& EntitySubsystem);
