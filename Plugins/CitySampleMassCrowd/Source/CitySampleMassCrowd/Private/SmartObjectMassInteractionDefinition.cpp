...
void USmartObjectMassInteractionDefinition::Activate(FMassCommandBuffer& CommandBuffer, const FMassBehaviorEntityContext& EntityContext) const
{
	...
	const FTransform SlotTransform = Subsystem.GetSlotTransform(SOUser.ClaimHandle).Get(FTransform::Identity);
	...
	if (MontageFragment != nullptr)
	{
		...
		CommandBuffer.PushCommand(FCommandAddFragmentInstance(Entity, FConstStructView::Make(TimedBehaviorFragment)));
	}
	else
	{
		FMassMontageFragment MontageData;
		MontageData.Request(AnimRequest);
		CommandBuffer.PushCommand(FMassCommandAddFragmentInstanceList(Entity,
			{
				FConstStructView::Make(MontageData),
				FConstStructView::Make(TimedBehaviorFragment)
			}));
	}
}
