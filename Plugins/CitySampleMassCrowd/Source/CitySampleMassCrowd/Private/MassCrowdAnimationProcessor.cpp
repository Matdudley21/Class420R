...
void UMassFragmentInitializer_Animation::Execute(UMassEntitySubsystem& EntitySubsystem, FMassExecutionContext& Context)
{
	check(World);
	const float GlobalTime = World->GetTimeSeconds();
	EntityQuery.ForEachEntityChunk(EntitySubsystem, Context, [this, GlobalTime](FMassExecutionContext& Context)
		{
...
void UMassProcessor_Animation::UpdateAnimationFragmentData(UMassEntitySubsystem& EntitySubsystem, FMassExecutionContext& Context, float GlobalTime, TArray<FMassEntityHandle, TInlineAllocator<32>>& ActorEntities)
{
	TArrayView<FCrowdAnimationFragment> AnimationDataList = Context.GetMutableFragmentView<FCrowdAnimationFragment>();
...
void UMassProcessor_Animation::UpdateVertexAnimationState(UMassEntitySubsystem& EntitySubsystem, FMassExecutionContext& Context, float GlobalTime)
{
	const int32 NumEntities = Context.GetNumEntities();
	TArrayView<FCrowdAnimationFragment> AnimationDataList = Context.GetMutableFragmentView<FCrowdAnimationFragment>();
	TConstArrayView<FMassMontageFragment> MontageDataList = Context.GetFragmentView<FMassMontageFragment>();
...
void UMassProcessor_Animation::UpdateSkeletalAnimation(UMassEntitySubsystem& EntitySubsystem, float GlobalTime, TArrayView<FMassEntityHandle> ActorEntities)
{
	...
	for (FMassEntityHandle& Entity : ActorEntities)
	{
		...
		const FMassMontageFragment* MontageFragment = EntitySubsystem.GetFragmentDataPtr<FMassMontageFragment>(Entity);
...
void UMassProcessor_Animation::Execute(UMassEntitySubsystem& EntitySubsystem, FMassExecutionContext& Context)
{
	check(World);
				...
				else
				{
					...
					for (int32 EntityIdx = 0; EntityIdx < NumEntities; EntityIdx++)
					{
						...
						if (AdjustedPositionPostAdvance > MontageLength)
						{
							...
							Context.Defer().PushCommand(FCommandRemoveFragment(Context.GetEntity(EntityIdx), FMassMontageFragment::StaticStruct()));
						}
						else
						{
...

	{
		...
		AnimationEntityQuery_Conditional.ForEachEntityChunk(EntitySubsystem, Context, [this, GlobalTime, &ActorEntities, &EntitySubsystem](FMassExecutionContext& Context)
			{
				UMassProcessor_Animation::UpdateAnimationFragmentData(EntitySubsystem, Context, GlobalTime, ActorEntities);
			});
	}
	{
		...
		AnimationEntityQuery_Conditional.ForEachEntityChunk(EntitySubsystem, Context, [this, GlobalTime, &EntitySubsystem](FMassExecutionContext& Context)
			{
				UMassProcessor_Animation::UpdateVertexAnimationState(EntitySubsystem, Context, GlobalTime);
			});
	}
...

	{
		...
		UMassProcessor_Animation::UpdateSkeletalAnimation(EntitySubsystem, GlobalTime, MakeArrayView(ActorEntities));
	}
}
